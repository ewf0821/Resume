# Script to check the output of the housekeeper program
#
import sys
import re
...
def get_num_threads(lines):
	...
	if ((washers != list(range(0,max(washers)+1))) or (dryers != list(range(0,max(dryers)+1))) or (both != list(range(0,max(both)+1)))): 
...
###
### Check that every thread does every activity 10 times and then exits
###
...
###
### Check that no washers/dryers are taken than are available
###
...
if max_num_washer_housekeepers_washing == 1 and num_washers_housekeepers > 1:
	print ("\tError: no more than 1 washer housekeeper uses the washer at a time!")
	exit(1)
