...
int compareString(const char *oneStr, const char *otherStr)
{
    ...
    while (oneStr[index] != NULL_CHAR && index < MAX_STR_LEN)
    {
        ...
        // check for difference bewtween characters
        if (diff != 0)
        {
            // return difference
            return diff;
    ...
    // end loop
    // return difference in string lengths, if any
...
void concatenateString(char *destStr, const char *sourceStr)
{
    ...
    // loop to end of destination string
    while (tempStr[sourceIndex] != NULL_CHAR && destIndex < MAX_STR_LEN - 1)
    {
        ...
        // set temporarty end of destination string
...
void copyString(char *destStr, const char *sourceStr)
{
    ...
    if (destStr != sourceStr)
    {
        ...
        while (sourceStr[index] != NULL_CHAR && index < MAX_STR_LEN - 1)
        {
            // assign characters to end of destination string
    ...
    char *capturedString)
{
    ...
    while ((intChar != EOF) && ((clearLeadingNonPrintable 
			&& intChar < (int)SPACE) || (clearLeadingSpace 
			&& intChar == (int)SPACE)))
    {
        // get next character
        // function: fgetc
...
/*
Name: toLowerCase
Process: if character is uppercase, sets it to lower case;
