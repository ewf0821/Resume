...
OpCodeType *addNode(OpCodeType *localPtr, OpCodeType *newNode)
{
    ...
    if (localPtr == NULL)
    {
        ...
        // assign all thre values to newly created node
...
OpCodeType *clearMetaDataList(OpCodeType *localPtr)
{
    ...
    if (localPtr != NULL)
    {
        // call recursive function with ext pointer
                 ...
                 OpCodeType **opCodeDataHead, char *endStateMsg)
{
    ...
    // initialie op code data pointer in case of return error
    *opCodeDataHead = NULL;
    ...
    if (getStringToDelimiter(fileAccessPtr, COLON, dataBuffer) || compareString(dataBuffer, "Start Program Meta-Data Code") != STR_EQ)
    {
        // close file
        // function: fclose
    ...
    //  (while conplete op commands are found)
    ...
    if (accessResult == LAST_OPCMD_FOUND_MSG)
    {
        ...
        if (startCount == endCount)
        {
            ...
            // function: getStringToDelimiter, compareString
            if (!getStringToDelimiter(fileAccessPtr, PERIOD, dataBuffer) || compareString(dataBuffer, "End Program Meta-Data Code") != STR_EQ)
            {
                // set access resilt to corrupted descriptor error
...
OpCodeMessages getOpCommand(FILE *filePtr, OpCodeType *inData)
{
    ...
    {
        // return last op command found mesage
    ...
    if (compareString(inData->command, "app") == STR_EQ && compareString(inData->strArg1, "start") == STR_EQ)
    {
        ...
        // set first int argument to mumber
...
{
    // checl for string holding three-letter op code command, return
    // function: compareString
    return (compareString(testCmd, "sys") == STR_EQ || compareString(testCmd, "app") == STR_EQ || compareString(testCmd, "cpu") == STR_EQ || compareString(testCmd, "mem") == STR_EQ || compareString(testCmd, "dev") == STR_EQ);
}
