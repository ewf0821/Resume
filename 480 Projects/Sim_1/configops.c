...
{
    // Define array woth eight ites, and short (10) lengths
                   ...
                   ConfigDataType **configData, char *endStateMsg)
{
    ...
    *configData = NULL;
    ...
    if (!getStringToDelimiter(fileAccessPtr, COLON, dataBuffer) || compareString(dataBuffer,
                                                                                 "Start Simulator Configuration File") != STR_EQ)
    {
        ...
        //  function: copyString
        copyString(endStateMsg, "Corrupt configuration file leader line error");
    ...
    // loop to end of config data item
    while (lineCtr < NUM_DATA_LINES)
    {
        ...
        if (dataLineCode != CFG_CORRUPT_PROMPT_ERR)
        {
            ...
            else if (dataLineCode == CFG_MD_FILE_NAME_CODE || dataLineCode == CFG_LOG_FILE_NAME_CODE || dataLineCode == CFG_CPU_SCHED_CODE || dataLineCode == CFG_LOG_TO_CODE || dataLineCode == CFG_MEM_DISPLAY_CODE)
            {
                // get string input
                // function: fscanf
        ...
        else
        {
            ...
            // function: copyString
            copyString(endStateMsg,
                       "Corrupted configuration prompt");
    ...
    // function getStringToDelimiter, compareString
    if (!getStringToDelimiter(fileAccessPtr, PERIOD, dataBuffer) || compareString(dataBuffer,
...
void stripTrailingSpaces(char *str)
{
    ...
    // loop while psace is found at end of string
                  ...
                  double doubleVal, const char *lowerCaseStrVal)
{
    ...
    case CFG_VERSION_CODE:
        ...
        // check if limits of version code are exceded
    ...
    // for cpu scheduing code
    case CFG_CPU_SCHED_CODE:

        // check for not finding one of the scheduling strings
        // function: compareString
        if (compareString(lowerCaseStrVal, "fcfs-n") != STR_EQ &&
            compareString(lowerCaseStrVal, "sjf-n") != STR_EQ &&
            compareString(lowerCaseStrVal, "srtf-p") != STR_EQ &&
            compareString(lowerCaseStrVal, "fcfs-p") != STR_EQ &&
            compareString(lowerCaseStrVal, "rr-p") != STR_EQ)
        {
            // set boolean result to false
            result = false;
        }
    ...
    case CFG_MEM_DISPLAY_CODE:
        ...
        // function: compareString
        if (compareString(lowerCaseStrVal, "on") != STR_EQ &&
            compareString(lowerCaseStrVal, "off") != STR_EQ)
        {
            // set boolean result to false
            result = false;
        }
    ...
    case CFG_MEM_AVAILABLE_CODE:
        ...
        // check for memory available limits exceeded
    ...
    case CFG_PROC_CYCLES_CODE:
        ...
        // check for process cycle limits exceeded
    ...
    // checl for lot to operation
    case CFG_LOG_TO_CODE:

        // check for not finding one of the log to strings
        // function: compareString
        if (compareString(lowerCaseStrVal, "both") != STR_EQ &&
            compareString(lowerCaseStrVal, "monitor") != STR_EQ &&
            compareString(lowerCaseStrVal, "file") != STR_EQ)
        {
            // set boolean result to false
            result = false;
        }
